...
###Create loops to go through all your global sets, and then the local sets too### 
###Create loops to go through all your global sets, and then the local sets too### 
sets=['TOP','TOPL','TOPR','BOTTOM','BOTTOML','BOTTOMR']
for interest in sets:
	gDispSet = []
	gCoordSet = []

	###Reading the Global Coordinates and Displacments
	gOdb = openOdb(path = 'global-1.odb')
	...
	for i in range(len(lSet)):
		...
		neighbourIndex = distSet.argsort()[:4][::1]
		...
		nodes1 = a.instances['Part-1-1'].nodes[lSet[i].label-1:lSet[i].label]
		...
		mdb.models['Model-1'].DisplacementBC(name='Node-'+str(lSet[i].label), 
			createStepName='Step-1',region=region, u1=lDispX, u2=lDispY, u3=lDispZ, ur1=UNSET, ur2=UNSET, ur3=UNSET, 
