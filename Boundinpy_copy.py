...
for i in part:
	...
	# 	print str(i)+'-'+str(n)+'mesh'
	# 	mdb.models['Model-1'].parts[str(i)+'-'+str(n)].PartFromMesh(copySets=True, name=
	# 	    	str(i)+'-'+str(n)+'-mesh')

		a = mdb.models['Model-1'].rootAssembly
		a.DatumCsysByDefault(CARTESIAN)
		x = mdb.models['Model-1'].parts[str(i)+'-'+str(n)+'-mesh']
		a.Instance(name=str(i)+'-'+str(n)+'-mesh-1', part=x, dependent=ON)

	# 	h=[]
	# 	n1 = a.instances[str(i)+'-'+str(n)+'-mesh-1'].nodes
	# 	name = n1.getByBoundingBox(xMin=0,yMin=0,xMax=1000,yMax=1000)
	# 	h.append(name)
